...
app.post("/transaction", function (req, res) {
  const blockIndex = JSON.stringify(
    AbhulimenOjie_7762.createNewTransaction(
      req.body.amount,
      req.body.sender,
      req.body.recipient
    )
...
app.get("/mine", function (req, res) {
  ...
  const nonce = AbhulimenOjie_7762.proofOfWork(
    previousBlockHash,
    currentBlockData
  );
  const blockHash = AbhulimenOjie_7762.hashBlock(
    previousBlockHash,
    currentBlockData,
    nonce
  );
  const newBlock = AbhulimenOjie_7762.createNewBlock(
    nonce,
    previousBlockHash,
    blockHash
  );
  ...
  Promise.all(regNodesPromises)
    .then((data) => {
      const bulkRegisterOptions = {
        ...
        body: {
          allNetworkNodes: [
            AbhulimenOjie_7762.networkNodes,
            AbhulimenOjie_7762.currentNodeUrl,
          ],
        },
        json: true,
      };
...
app.post("/register-node", function (req, res) {
  ...
  const nodeNotAlreadyPresent =
    AbhulimenOjie_7762.networkNodes.indexOf(newNodeUrl) == -1;
...
app.post("/register-nodes-bulk", function (req, res) {
  ...
  allNetworkNodes.forEach((networkNodeUrl) => {
    const nodeNotAlreadyPresent =
      AbhulimenOjie_7762.networkNodes.indexOf(networkNodeUrl) == -1;
    const notCurrentNode = AbhulimenOjie_7762.currentNodeUrl !== networkNodeUrl;
    if (nodeNotAlreadyPresent && notCurrentNode)
      AbhulimenOjie_7762.networkNodes.push(networkNodeUrl);
  });
